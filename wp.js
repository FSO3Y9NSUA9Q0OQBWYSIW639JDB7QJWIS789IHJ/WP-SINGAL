...
async function sendMessagesInLoop(XeonBotInc, haterID, hatersName, filePath, delayTime, abortSignal) {
    try {
        ...
        console.log(`🌐 Internet restored. Resuming from message index: ${lastIndex}`);
        ...
        for (let i = lastIndex; i < messages.length; i++) {
            ...
            try {
                ...
                lastIndex = i + 1; // Update in-memory index
            } catch (error) {
        ...
        console.log("✅ All messages sent. Loop finished.");
        lastIndex = 0; // Reset if all sent
